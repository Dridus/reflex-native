...
public:
  ...
  */
  kiwi::Error* addConstraint( const Constraint& constraint )
  {
    if( m_cns.find( constraint ) != m_cns.end() )
      return new kiwi::Error(kiwi::DuplicateConstraint);
    ...
    if( subject.type() == Symbol::Invalid && allDummies( *rowptr ) )
    {
      if( !nearZero( rowptr->constant() ) )
        return new kiwi::Error(kiwi::UnsatisfiableConstraint);
      else
        subject = tag.marker;
    }
  ...
  */
  kiwi::Error* removeConstraint( const Constraint& constraint )
  {
    CnMap::iterator cn_it = m_cns.find( constraint );
    if( cn_it == m_cns.end() )
      return new kiwi::Error(kiwi::UnknownConstraint);
    ...
    else
    {
      ...
      if( row_it == m_rows.end() )
        return new kiwi::Error(kiwi::InternalSolverError, "failed to find leaving row");
  ...
  */
  kiwi::Error* addEditVariable( const Variable& variable, double strength )
  {
    if( m_edits.find( variable ) != m_edits.end() )
      return new kiwi::Error(kiwi::DuplicateEditVariable);
    strength = strength::clip( strength );
    if( strength == strength::required )
      return new kiwi::Error(kiwi::BadRequiredStrength);
  ...
  */
  kiwi::Error* removeEditVariable( const Variable& variable )
  {
    EditMap::iterator it = m_edits.find( variable );
    if( it == m_edits.end() )
      return new kiwi::Error(kiwi::UnknownEditVariable);
  ...
  */
  kiwi::Error* suggestValue( const Variable& variable, double value )
  {
    EditMap::iterator it = m_edits.find( variable );
    if( it == m_edits.end() )
      return new kiwi::Error(kiwi::UnknownEditVariable);
...
private:
  ...
  */
  kiwi::Error* optimize( const Row& objective )
  {
    while( true )
    {
      ...
      if( it == m_rows.end() )
        return new kiwi::Error(kiwi::InternalSolverError, "The objective is unbounded.");
  ...
  kiwi::Error* dualOptimize()
  {
    while( !m_infeasible_rows.empty() )
    {
      ...
      if( it != m_rows.end() && it->second->constant() < 0.0 )
      {
        ...
        if( entering.type() == Symbol::Invalid )
          return new kiwi::Error(kiwi::InternalSolverError, "Dual optimize failed.");
        // pivot the entering symbol into the basis
        Row* row = it->second;
        m_rows.erase( it );
        row->solveFor( leaving, entering );
